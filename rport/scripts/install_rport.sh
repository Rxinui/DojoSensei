...
# used by Dockerfile.
#
# This version install rport-client on VM but does not test the connection with rportd server.
#
# @see https://github.com/cloudradar-monitoring/rport-pairing/tree/main/retrieve/templates/linux
# @author Rxinui
##

## BEGIN @Rxinui custom
. ./scripts/env_rport.sh
# FINGERPRINT=`curl -k -s https://rport:5000/fingerprint.txt`
# CONNECT_URL="http://rport:8000"
# CLIENT_ID="dojo-api_vbox-1"
# PASSWORD="auauNZgEiVxB"
## END @Rxinui custom

####### Generated by https://github.com/cloudradar-monitoring/rport-pairing/tree/main/retrieve/templates/linux #######
...
EOF
    ...
    rc-service rport start # @Rxinui avoid connection error when installing rport
...
install_client() {
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
        echo "Dry run activated: try to reach rportd server"
        test_connection
    else
...
SELINUX_FORCE=0
DRY_RUN=0
while getopts 'hvfcsuxstilda:' opt; do
    case "${opt}" in
    ...
    i) INSTALL_TACO=1 ;;
    l) SELINUX_FORCE=1 ;;
    # Execute a single function. For dev and debug only  #c
    a) USER=${OPTARG} ;;
    d) DRY_RUN=1 ;;
